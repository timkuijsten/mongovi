 ...
 */
static int
test_shorten(const char *input, const int maxlen, const char *exp,
    const int exp_exit, const char *msg)
{
	int exit;
	char str[MAXSTR];
 ...
 */
static int
test_shorten_comps(const char *comp1, const char *comp2, const int maxlen,
    const char *exp1, const char *exp2, const int exp_exit, const char *msg)
{
	int exit;
	char c1[MAXSTR];
	char c2[MAXSTR];
	...
	if (strcmp(c1, exp1) == 0 && strcmp(c2, exp2) == 0) {
		if (verbose)
			printf("PASS: %s %s %d = \"%s\" \"%s\" \t%s\n", comp1,
			    comp2, maxlen, c1, c2, msg);

		return 0;
	} else {
		fprintf(stderr, "FAIL: %s %s %d = \"%s\" and \"%s\" instead of "
		    "\"%s\" and \"%s\" \t%s\n", comp1, comp2, maxlen, c1, c2,
		    exp1, exp2, msg);
...
main(void)
{
	...
	failed += test_shorten_comps("foobarbaz", "quxquuzraboof", 23,
	    "foobarbaz", "quxquuzraboof", 22, "");
	failed += test_shorten_comps("foobarbaz", "quxquuzraboof", 22,
	    "foobarbaz", "quxquuzraboof", 22, "");
	failed += test_shorten_comps("foobarbaz", "quxquuzraboof", 21,
	    "foobarbaz", "quxqu..aboof", 21, "");
	failed += test_shorten_comps("foobarbaz", "quxquuzraboof", 20,
	    "foobarbaz", "quxqu..boof", 20, "");
	failed += test_shorten_comps("foobarbaz", "quxquuzraboof", 19,
	    "foo..az", "quxqu..aboof", 19, "");
	failed += test_shorten_comps("foobarbaz", "quxquuzraboof", 18,
	    "foo..az", "quxqu..boof", 18, "");
	failed += test_shorten_comps("foobarbaz", "quxquuzraboof", 17,
	    "foo..az", "quxq..boof", 17, "");
	failed += test_shorten_comps("foobarbaz", "quxquuzraboof", 16,
	    "fo..az", "quxq..boof", 16, "");
	failed += test_shorten_comps("foobarbaz", "quxquuzraboof", 15,
	    "fo..az", "quxq..oof", 15, "");

	failed += test_shorten_comps("foobarbaz", "quxquuzraboof", 9, "f..z",
	    "qu..f", 9, "");
	failed += test_shorten_comps("foobarbaz", "quxquuzraboof", 8, "f..z",
	    "q..f", 8, "");
	...
	failed += test_shorten_comps("＄£ह€", "한𐍈＄£ह€한𐍈＄", 8, "..ह€",
	    "한..", 8, "");
	failed += test_shorten_comps("＄£ह€", "한𐍈＄£ह€한𐍈＄", 9, "＄£ह€",
	    "한..", 9, "");
	failed += test_shorten_comps("＄£ह€", "한𐍈＄£ह€한𐍈＄", 10, "＄£ह€",
	    "한𐍈..", 10, "");
	failed += test_shorten_comps("＄£ह€", "한𐍈＄£ह€한𐍈＄", 11, "＄£ह€",
	    "한..＄", 11, "");
	failed += test_shorten_comps("＄£ह€", "한𐍈＄£ह€한𐍈＄", 12, "＄£ह€",
	    "한𐍈..＄", 12, "");
	failed += test_shorten_comps("＄£ह€", "한𐍈＄£ह€한𐍈＄", 13, "＄£ह€",
	    "한𐍈..𐍈＄", 13, "");
	failed += test_shorten_comps("＄£ह€", "한𐍈＄£ह€한𐍈＄", 14, "＄£ह€",
	    "한𐍈..𐍈＄", 13, "");
